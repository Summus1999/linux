...
> 适用对象：已有鸿蒙内核协议栈开发经验，希望将 ARP 相关知识平移到 Linux 内核网络协议栈。
>
> 重点文件：
...
```text
主机 A (192.168.1.2) 想发给 192.168.1.3
→ A 不知道 1.3 的 MAC
→ A 发 ARP Request（广播：谁有 192.168.1.3？）
...
```text
| 以太网头 (14B) | arphdr (8B) | Sender MAC/SHA (6B) | Sender IP/SIP (4B) | Target MAC/THA (6B) | Target IP/TIP (4B) |
```
...
`probes` 的判定容易误读：`arp_solicit()` 只有在 `atomic_read(&neigh->probes) < UCAST_PROBES` 时才用已有 `ha` 做 unicast probe。首次从 `NUD_NONE` 进入 `NUD_INCOMPLETE` 时，`__neigh_event_send()` 会先把 `probes` 设为 `UCAST_PROBES`，所以通常直接走广播 ARP Request，而不是先发 unicast。
...
这是 ARP 包处理的“大脑”。下面只保留来自 `net/ipv4/arp.c` 的原始源码片段，解释放在代码块外。
...
关键：**组播、广播、lo、P2P、NOARP 设备不需要发 ARP**，直接 `NUD_NOARP`。
...
| 维度 | Linux | 鸿蒙（轻内核 / 标准内核） |
|------|-------|--------------------------|
| ARP 缓存管理 | 统一在 `neighbour` 子系统，按协议抽象 | 通常 ARP 模块自己维护表 |
| 状态机 | NUD 状态机通用化 | 可能有独立的 ARP 状态机 |
| sk_buff | 非常复杂：frag、shared info、GSO/GRO | 通常更轻量 |
| 发送触发 | 由 `neigh->output` 懒解析 | 可能由 IP 层直接调 ARP 模块 |
| 锁机制 | RCU + `neigh->lock` + `tbl->lock` | 视具体实现 |
| Netfilter | ARP 也走 NF_HOOK | 可能没有 |
| sysctl | 大量运行时参数 | 较少 |
| 多网 namespace | `net_namespace` 隔离 | 通常无 |

**学习建议**：看 Linux ARP 时，不要只盯着 `arp.c`，要把 **neighbour.c 的状态机** 一起看完，否则理解不了“为什么收到 Reply 后只是调 `neigh_update`”。

---

## 10. 推荐学习路径

1. **先通读 `net/ipv4/arp.c`**：重点 `arp_rcv()` → `arp_process()` → `arp_send()` → `arp_create()` → `arp_xmit()`。
2. **再读 `net/core/neighbour.c`**：重点 `neigh_lookup()`、`neigh_update()`、`neigh_resolve_output()`、`neigh_event_send()`、`neigh_timer_handler()`。
3. **画一张调用链图**：从 `ip_queue_xmit()` 到 `arp_xmit()`，再从 `arp_rcv()` 到 `neigh_update()`。
4. **做实验**：
   ```bash
   ip neigh show
   cat /proc/net/arp
...
- ARP 报文经过 `NF_ARP_IN` / `NF_ARP_OUT` 时，nftables/iptables 能做什么？
- `arp_solicit` 里 unicast probe 和 multicast probe 的切换条件是什么？
- 当 neighbour 状态是 `NUD_STALE` 时，IP 包发出去会发生什么？
- IPv6 不用 ARP，用 NDISC，`net/ipv6/ndisc.c` 与 `arp.c` 如何对位？
...
**逐行解释**：
- 默认情况下，要求 `ar_pro == ETH_P_IP` 且 `ar_hrd == dev->type`。
...
### 12.5 第四阶段：处理 ARP Request

#### 12.5.1 DAD（重复地址检测）特殊处理
...
### 12.6 第五阶段：处理 ARP Reply / 更新 ARP 表
...
### 14.2 `neigh_resolve_output()`：解析入口
...
### 14.3 `__neigh_event_send()`：状态机启动与排队
...
### 14.5 `neigh_timer_handler()`：定时器超时与重传
...
### 14.6 解析成功：`neigh_update()` 与 `process_arp_queue`
...
应用层        sendto()
  │              │
  ▼              ▼
IP 层      ip_queue_xmit(skb, dst=192.168.1.3)
  │              │
  │              ▼
...
## 17. 与鸿蒙的再对比：为什么 Linux 这样设计？

| 设计点 | Linux 思路 | 鸿蒙常见思路 | 优劣 |
|--------|-----------|-------------|------|
| 缓存归属 | neighbour 子系统统一管 | ARP 模块自管 | Linux 更通用，支持 IPv6 ND、ATM 等；鸿蒙更直接 |
| 发包与解析耦合 | IP 包触发 `neigh_resolve_output`，自动排队 | 可能先解析再发包 | Linux 对应用透明，但首次有延迟 |
| 状态机 | NUD 通用状态机 | ARP 专用状态机 | Linux 复用性强，但学习曲线陡 |
| 队列 | 每个 neighbour 一个 `arp_queue` | 可能全局队列或简单丢弃 | Linux 能缓存多个包，但耗内存 |
| 定时器 | 每个 neighbour 一个 timer | 可能轮询或集中定时器 | Linux 精确但 timer 数量多 |

理解 Linux ARP 的关键不是背函数，而是理解：**IP 包和 ARP 请求是并行的——IP 包被挂起，ARP 独立探测，成功后唤醒队列。**

---

## 18. 下一步学习建议

1. **动手实验**：在虚拟机里 `ip neigh flush all` 后 ping 网关，同时 `tcpdump -nn arp`，观察第一次解析。
2. **改代码实验**：在 `arp_solicit()` 里加 `pr_info()`，看 probes 变化和 unicast/multicast 切换。
3. **对比 IPv6**：读 `net/ipv6/ndisc.c` 的 `ndisc_recv_ns()` / `ndisc_send_ns()`，看同样的 neighbour 框架如何复用。
4. **看连接失败路径**：模拟 ARP 解析失败（如 ping 一个不存在的同网段 IP），跟踪 `neigh_invalidate()` → `arp_error_report()` → TCP 如何感知。
